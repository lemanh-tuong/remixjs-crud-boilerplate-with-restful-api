...
type RawChartJSProps = ComponentProps<typeof Pie>;

export interface Props<Label extends string, DataSetRawData, SliceRawData> {
  /**
   * An array of labels representing each slice of the pie chart.
  ...
  /**
   * An array of datasets to be visualized in the pie chart.
...
/**
 * A PieChart component that renders data as a pie chart, where each slice
 * represents a proportion of the whole based on numerical values.
 *
 * @param labels - The labels for the slices in the pie chart.
 * @param datasets - The datasets containing slices to be displayed in the chart.
 * @param formatter - A function to format the numerical values displayed on the slices.
 * @param onClickSlice - A callback function triggered when a slice is clicked.
 * @param onHoverSlice - A callback function triggered when a slice is hovered over.
 * @param className - An optional CSS class for custom styling of the chart.
 * @param tooltip - An object to customize the tooltip displayed when hovering over a slice.
 *
 * @returns A ReactNode representing the rendered pie chart.
 */
export const PieChart = <Label extends string, DataSetRawData, SliceRawData>({
  labels,
  datasets,
  formatter,
  onClickSlice,
  ...
  return (
    <ClientSideOnly>
      <div
        className={classNames('AntPieChart__container', className)}
        onMouseLeave={event => {
      ...
      >
        ...
        >
          <div
            className="AntPieChart__tooltipPlaceholder"
            style={{
              left: tooltipState?.x,
              top: tooltipState?.y,
            }}
