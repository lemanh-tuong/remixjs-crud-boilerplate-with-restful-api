...
export const getBrandingsForDefer = async ({
  remixRequest,
  page,
  pageSize,
  searcher,
  ...
  await delay(3000);
  return getFetchApiInstance(remixRequest)
    .then(fetchApi => {
      return fetchApi.request<ResponseListSuccess<ResponseData> | ResponseFailure>({
